...
class ToggleFavouriteView(LoginRequiredMixin, View):
    """Добавя или премахва продукт от любими"""
    ...
    def get(self, request, product_id):
        ...
        if product in request.user.favourites.all():
            ...
            messages.success(request, f'Продуктът "{product.name}" беше премахнат от любими.')
        else:
            request.user.favourites.add(product)
            messages.success(request, f'Продуктът "{product.name}" беше добавен към любими.')
        
        return redirect('products:product_detail', pk=product_id)


class FavouriteListView(LoginRequiredMixin, TemplateView):
    """Списък с любими продукти на потребителя"""
